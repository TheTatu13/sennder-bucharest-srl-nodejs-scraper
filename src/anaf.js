 ...
 *
 * demoanaf.ro now requires payment (returns 402 on every call), so every
 * live lookup here fell through to the caller's cache — fine for a real
 * scrape run (getCompanyFromANAFWithFallback has a cache), but the
 * integration tests call getCompanyFromANAF/searchCompany directly and
 * have no cache to fall back to, so they failed outright. Mirrors the
 * Brewtality-3-16 template's scraper/anaf.js fix for the same issue.
 */
